...
static void thread_recv_func(void* data)
{
	...
	pdcb = &gat_UartDcb[0];
	if(mu32_RecFlag) {													// ���ڽ��ձ�־
		if( _yIs1msTimeout(mu32_RecStartTime, mu32_RecUseTime) ) {		// �����Ƿ����
		...
		if( isfram ) {													// ��һ������
            ...
            if( pdcb->flag.UseRx ) {                                // ��ǰʹ��1�Ż�����
                if(pdcb->flag.Rx0Flg ==0) {                         // 0�Ż���������δʹ��
                    pdcb->aRxCnt[1] = pdcb->RxCnt;                  // ���ո�������
                    pdcb->flag.Rx1Flg =2;                           // ��1�Ż�������������
                    if(pdcb->flag.HaveRxMsg <2)
                        pdcb->flag.HaveRxMsg++;                     // ������������־
                    pdcb->pRxBuf = pdcb->paRxBuf[0];                // ʹ��0�Ż�����
                    pdcb->flag.UseRx =0;                            // ����ʹ��0�Ż�������־
                }
            }
            else {
                if(pdcb->flag.Rx1Flg ==0) {                         // 1�Ż���������δʹ��
                    pdcb->aRxCnt[0] = pdcb->RxCnt;                  // ���ո�������
                    pdcb->flag.Rx0Flg =2;                           // ��0�Ż�������������
                    if(pdcb->flag.HaveRxMsg <2)
                        pdcb->flag.HaveRxMsg++;                     // ������������־
                    pdcb->pRxBuf = pdcb->paRxBuf[1];                // ʹ��1�Ż�����
                    pdcb->flag.UseRx =1;                            // ����ʹ��1�Ż�������־
                }
            }
            pdcb->RxCnt = 0;                                        // ���Ϊ�´ν���׼��
            pdcb->flag.isRxing = 0;                                 // ������ڽ��ձ�־				
		}		
	}
	else {																// δ���ڽ��ձ�־
...
static void thread_send_func(void* data)
{
	...
	pdcb = &gat_UartDcb[0];
	if(mu32_SdFlag) {													// ���ڷ���״̬
		if( _yIs1msTimeout(mu32_SendStartTime, mu32_SendUseTime) ) {	// �������
			pdcb->flag.isTxing = 0;                                 	// ������ڷ��ͱ�־	
			mu32_SdFlag = 0;											// ������ڷ���״̬
		}
		
	}
	else if(pdcb->Txflg >0) {											// �Ѿ�������Ҫ����
		pdcb->Txflg = 0;
